...
function onCountItemsLoaded(items) {
    ...
    for (i = 0; i < currentCountItems.length; i++) {
        currentCountItems[i].currentStock = currentCountItems[i].stockAtCount;
    }
    applyDetailFilters();
}
...
function applyDetailFilters() {
    ...
    renderInventoryGrid(filtered, false, "#countDetailContainer");
}
...
function onSetPrimaryClick() {
    primaryToggleTarget = !currentCountIsPrimary;
    ajaxCall(
        "PUT",
        "/inventorycounts/" + currentCountId + "/primary?branchCode=" + currentBranchCode + "&isPrimary=" + primaryToggleTarget,
        null,
        onSetPrimaryDone,
        onSetPrimaryError
    );
...
function onSetPrimaryDone() {
    currentCountIsPrimary = primaryToggleTarget;
...
function onDetailStockChange() {
    ...
    if (isNaN(newStock) || newStock < 0) {
        Swal.fire({ title: "ערך שגוי", text: "הכמות חייבת להיות 0 או יותר.", icon: "error" });
        for (i = 0; i < currentCountItems.length; i++) {
            if (currentCountItems[i].productId === productId) {
                input.val(currentCountItems[i].currentStock);
                break;
            }
        }
        return;
    ...
    for (i = 0; i < currentCountItems.length; i++) {
        if (currentCountItems[i].productId === productId) {
            currentCountItems[i].currentStock = newStock;
            required = currentCountItems[i].requiredStock || 0;
            break;
        }
    }
...
function onSaveCountEditClick() {
    var payload = {
        ...
        branchCode: currentBranchCode,
        items: collectCountEditItems()
    };
    ajaxCall("PUT", "/inventorycounts/" + currentCountId + "/items", payload, onCountEditSaved, onCountEditSaveError);
}

function collectCountEditItems() {
    var items = [];
    var i;
    for (i = 0; i < currentCountItems.length; i++) {
        items.push({
...
function onCountEditSaveError() {
    Swal.fire({ title: "שגיאה", text: "לא ניתן לעדכן את הספירה.", icon: "error" });
}
...
function onDeleteCountClick(countId, countName) {
    Swal.fire({
        ...
        if (!result.isConfirmed) return;
        ajaxCall("DELETE", "/inventorycounts/" + countId, null, onCountDeleted, onCountDeleteError);
    });
}

function onCountDeleted() {
    Swal.fire({ title: "נמחק!", text: "הספירה נמחקה.", icon: "success", timer: 1500, showConfirmButton: false });
    loadCountsList();
}

function onCountDeleteError() {
    Swal.fire({ title: "שגיאה", text: "לא ניתן למחוק את הספירה.", icon: "error" });
}
